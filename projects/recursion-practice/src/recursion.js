...
var sum = function(array) {
  //base case if array is empty return 0 and end the recursive call
  if(array.length === 0){
    return 0;
  } 

  //recursive call: add first value to recursive call
...
var sumBelow = function(n) {
  //base case
  var step = n > 0 ? 1 : -1; //step to go up or down for neg or pos num
  if(n === 0){
    return 0
  }
  //recursive call
...
var reverseArr = function (array) {
  //base case if array length is 0 return array
  if (array.length === 0) {
    return array;
  };
  //recursion: starting at 2nd index concat array with 
...
var countOccurrence = function(array, value) {
  //base case: return 0 once array length is 0;
  if(!array.length){
    return 0;
  }
  //recursive call
...
var numToText = function(str, returnStr = "") {
  ...
  //recursive call
  //if current val is not a digit num add the curr val to the return str default
  if(!Object.keys(nums).includes(str[0])){
    returnStr += str[0]
  } else {
    var newStr = nums[str[0]] + str.slice(1)
    return numToText(newStr, returnStr);
  }
